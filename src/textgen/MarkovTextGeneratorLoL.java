...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText) {
		...
		if (sourceText.length() == 0) {
			...
			String[] splittedStr = sourceText.split("[\\s]+");
			starter = splittedStr[0];
			...
			for (int i = 1; i < splittedStr.length; i++) {
				...
				// Use helper method to check if prevWord already exist and if
				// so
				// add next work
				// and if not create new node
			...
			// for last word check if it already exist and if so add next work
			// and if not create new node
			node = findNode(prevWord);
			if (node == null) {
				node = new ListNode(prevWord);
